...
def ui_reset_date_filter(date, reset):
    if date:
        if reset.button(
            "📅 Max date applied. **Click to reset to date and time**",
            use_container_width=True,
            type="primary",
        ):
            date = None
...
def ui_aggregated_sensor_data(data_now, is_bike=False):
...
def ui_tab_car(tab):
    with tab:
        ...
        if data_now is None:
            ...
            \n ⌚ **Showing data from**: `{data_now.at[0, 'date']}` (**updated every hour**)"""
